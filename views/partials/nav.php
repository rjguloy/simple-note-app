        ...
        <div class="hidden md:block">
            <div class="ml-4 flex items-center md:ml-6">
                ...
                <div class="relative inline-block ml-3">
                    <?php if ($_SESSION['user'] ?? false) : ?>
                        <button id="user-menu-button" class="relative flex max-w-xs items-center rounded-full 
                        focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">
                        <!-- <span class="sr-only">Open user menu</span> -->
                            <img src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e"
                                class="size-8 rounded-full outline -outline-offset-1 outline-white/10" />
                        </button>
                        <!-- <form method="POST" action="/sessions" class="inline ml-3">
                            <input type="hidden" name="_method" value="delete" />
                            <button class="text-white inline rounded-md px-3 py-2 text-sm font-medium  hover:bg-white/5 hover:text-white">Log Out</button>        
                        </form> -->
                    <?php else : ?>
                ...
                <?php if ($_SESSION['user'] ?? false) : ?>
                <div class="ml-3">
                    <form method="POST" action="/session" class="inline-block">
                        <input type="hidden" name="_method" value="delete" />
                        <button class="text-gray-300 rounded-md px-3 py-2 text-sm font-medium  hover:bg-white/5 hover:text-white">Log Out</button>        
                    </form>
                </div>
            <?php endif; ?>
            </div>
